...
if(CACHE)
{
	...
	{
		$long_url = mysql_result(mysql_query('SELECT long_url FROM ' . DB_TABLE . ' WHERE code="' . mysql_real_escape_string($shortened_id) . '"'), 0, 0);
...
if(TRACK)
{
	mysql_query('UPDATE ' . DB_TABLE . ' SET referrals=referrals+1 WHERE code="' . mysql_real_escape_string($shortened_id) . '"');
}
